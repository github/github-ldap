...
class GitHubLdapConnectionCacheTestCases < GitHub::Ldap::Test
  ...
  def test_get_connection
    conn = GitHub::Ldap::ConnectionCache.get_connection({:host => "host"})
    assert_equal GitHub::Ldap, conn.class
  end

  def test_returns_cached_connection
    conn1 = GitHub::Ldap::ConnectionCache.get_connection({:host => "ghe.local"})
    conn2 = GitHub::Ldap::ConnectionCache.get_connection({:host => "ghe.local"})
